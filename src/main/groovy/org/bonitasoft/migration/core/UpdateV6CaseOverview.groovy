...
AND key_ like 'processInstance/%'""" // KEY_ like 'processInstance/%' means 'case overview'
    ...
    void run(boolean isSp) {
        if (getMappingIdAndTenantId()) {
            ...
            updateV6CaseOverviews()
            displayWarningAfterOperation()
        }
    }
    ...
    void updateV6CaseOverviews() {
        context.sql.executeUpdate(UPDATE_CASE_OVERVIEW_PAGE_MAPPINGS, tenantId, mappingId, tenantId)
        context.sql.executeUpdate(UPDATE_CASE_OVERVIEW_FORM_MAPPINGS, processDefinition, tenantId)
    }

    private void displayWarningAfterOperation() {
        String[] output = ["The replacement of the case overview by autogenerated V7 case overview has succeeded on the following process :",
                           "id: ${processDefinition}",
                           "name: ${processName}",
                           "version: ${processVersion}",
                           "tenant Id: ${tenantId}"]
        displayUtil.logWarningsInRectangleWithTitle("Updated Case Overview", output)
    }

    @Override
    List<String> getBannerAndGlobalWarnings() {
        return [
                "The migration tool was executed using 'updateCaseOverview' option, it will update the V6 case overview to autogenerated V7 case overview of a given process, allowing to migrate to Bonita 7.8.0+",
