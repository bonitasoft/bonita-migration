...
import org.bonitasoft.migration.version.to7_13_0.CreateNewPages
...
class RemoveHiddenFieldFromPageIT extends Specification {
    ...
    private RemoveHiddenFieldFromPages migrationStep = new RemoveHiddenFieldFromPages()
    ...
    def "should remove hidden field from pages"() {
        given:
        assert migrationContext.databaseHelper.hasColumnOnTable("page", "hidden")

        when:
        migrationStep.execute(migrationContext)

        then:
        !migrationContext.databaseHelper.hasColumnOnTable("page", "hidden")
    }
}
