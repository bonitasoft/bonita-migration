...
class SplitRestSecurityConfigIT extends Specification {
    ...
    def "should migrate resources-permissions-mapping.properties"() {
        ...
        customProps.normalize() == expectedCustomProps.normalize()
    ...
    def "should not fail if config file is empty (empty == NULL on Oracle)"() {
        given:
        migrationContext.sql.execute("""
            INSERT INTO configuration (tenant_id, content_type, resource_name, resource_content)
            VALUES (2, 'TENANT_PORTAL', 'compound-permissions-mapping.properties', ${'# dummy line'.bytes})
            """)

        when:
        new SplitRestSecurityConfig().execute(migrationContext)

        def defaultProps = dbUnitHelper.getBlobContentAsString(migrationContext.sql.firstRow("""
            SELECT resource_content FROM configuration WHERE tenant_id = 2 and content_type = 'TENANT_PORTAL'
            AND resource_name = 'compound-permissions-mapping.properties'""")["resource_content"])

        then:
        defaultProps.normalize() == '\n' // empty content has been replaced by Newline
    }
