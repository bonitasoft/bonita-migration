...
-- TABLE profile is here only for foreign key integrity check:
...
-- TABLE page is here only for foreign key integrity check:
CREATE TABLE page (
  tenantId BIGINT NOT NULL,
  id BIGINT NOT NULL,
  name VARCHAR(255) NOT NULL,
...
-- those constraints are here for foreign key integrity check when dropping the indexes before recreating them:
...
ALTER TABLE job_param ADD CONSTRAINT fk_job_param_jobid FOREIGN KEY (tenantid, jobDescriptorId) REFERENCES job_desc(tenantid, id) ON DELETE CASCADE; -- need to be recreated without tenantid first
